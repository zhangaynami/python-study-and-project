...
# kmeans = KMeans(n_clusters=3, random_state=0).fit(normalized_df[['L', 'R', 'F', 'M', 'C']])
# print(kmeans)
# 计算每个会员的LRFMC得分
normalized_df['LRFMC'] = normalized_df['L'] + normalized_df['R'] + normalized_df['F'] + normalized_df['M'] + normalized_df['C'].mean()
k = 5
#初始化Kmeans模型
...
# print(kmeansResult)
# ['LRFMC'] = kmeans.labels_
# print(normalized_df)
# 将会员按LRFMC得分从高到低排序
sorted_df = normalized_df.sort_values('LRFMC', ascending=False)

# 将会员分为5组
group_labels = ['群体一', '群体二', '群体三', '群体四', '群体五']
sorted_df['Group'] = pd.qcut(sorted_df['LRFMC'], 5, labels=group_labels)

# 计算每组的平均得分
grouped = sorted_df.groupby('Group').mean()
# print(kmeansResult)
# print(grouped)
group = pd.merge(kmeansResult,grouped,on = 'Group')
# print(group)
groupeds = group[["L_x","R_x","F_x","M_x","C_x"]]
# 绘制雷达图
print(groupeds)
categories = list(groupeds.index)
